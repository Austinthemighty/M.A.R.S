<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>News</title>
    <link rel="icon" type="image/jpg" href="images/img3.jpg">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" href="Mars_Reservation.css">
    <link rel="stylesheet" href="dropDown.css">
    <script type="text/javascript">

        var slideimages = new Array()
        slideimages[0] = new Image()
        slideimages[0]
    </script>
    <title>M.A.R.S.</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" href="Mars_Reservation.css">
</head>
<body>

<label for="show-menu" class="show-menu">Show Menu</label>
<input type="checkbox" id="show-menu" role="button">

<ul id="menu">
    <li><a href="index.php">Home</a></li>
    <li>
        <a href="news.php">News</a>
    </li>
    
    <li><a href="Mars_Reservation.php">Reservation ￬</a>
        <ul class="hidden">
            <li><a href="#">Make Reservation</a></li>
            <li><a href="#">Edit Reservation</a></li>
        </ul>
    </li>
    <li><a href="contact.php">Contact</a></li>
    <li>
        <a href="About.php">About</a>

    </li>
</ul>
<div style="width: 75%; height: 100%; float: left">
    <div style="width: 75%; height: 100%; background-color: hsla(0, 0%, 100%, 0.75);; float: right" >
<br>
<br>
<br>
<table border="1">
    <tr> <!--shows a banner for the news headline-->
    <td colspan="2" style="width: 100%"><img src="images/destination-mars-banner%202.jpg" width="100%" height="50px"></td>
    <tr> <!--shows apicture for the news story-->
        <td style="width: 150px"><img src="http://www.quotehd.com/imagequotes/authors16/tmb/jeff-williamson-quote-so-far-there-are-no-complaints.jpg" width="150px" height="150px"></td>
        <td>
            <h1 style="text-align: center">0 Complaints so far</h1><br> <!--Headline-->
            <p>Of the many increments of time our company has been in business, we have received 0 formally filed complaints, we are committed to continuously great service to keep that number at 0</p>
        </td> <!-- ^The actual news-->
    </tr>
</table>
<table border="1"> <!--copy of the last table but changes the image and text for a new headline-->
    <tr>
        <td colspan="2" style="width: 100%"><img src="images/destination-mars-banner%202.jpg" width="100%" height="50px"></td>
    </tr>
    <tr>
        <td style="width: 150px"><img src="http://affiliatetip.com/wp-content/uploads/2012/02/rocket-science.jpg" width="100%" height="150px"></td>
        <td>
            <h1 style="text-align: center">Mission: the Red planet</h1><br>
            <p>Scientist are advancing the technology required to send people to mars, so make your reservations quickly, you don't want to miss it</p>
        </td>
    </tr>
</table>
        </div></div>
</body>
</html>