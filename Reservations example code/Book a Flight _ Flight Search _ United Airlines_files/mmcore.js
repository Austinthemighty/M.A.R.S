...
if(!window.mmcore){
	window.mmcore={
		...
		SetCookie:function(n,v,d,g){var _t=this,_h=_t._Host(_t._TL(location.hostname)),exp=_t._FutureDate(d);
			_t._d.cookie=escape(eval(g)?n:_t._PN(n))+'='+escape(v)+(_h.length?';domain='+(g?(navigator.userAgent.match(/MSIE/)?location.host:''):_h):'')
...
//private
	...
	if(location.hostname.indexOf('united.com')+1) mmcore.cookie_domain = '.united.com';
